...
echo
read -rsp "🔐 Contraseña para interfaz web (se usará para generar hash): " WG_PASSWORD
echo
...
# Generar hash
echo "🔐 Generando hash seguro..."
PASSWORD_HASH=$(pct exec "$CT_ID" -- htpasswd -nbBC 12 admin "$WG_PASSWORD" | cut -d: -f2)
...
echo -e "💻 Acceso interno: pct enter $CT_ID"
echo -e "🔐 Usuario root / contraseña: la que ingresaste"
echo -e "\n🌐 Interfaz web local: http://$CT_IP_ONLY:$WG_ADMIN_PORT"
echo -e "🌍 Interfaz desde Internet: http://$WG_HOST:$WG_ADMIN_PORT"
echo -e "👤 Usuario: admin"
echo -e "🔐 Contraseña: la que escribiste (hasheada internamente)"
